...
for i in {1..16}; do
    ...
    for T_LOC in ${T_LOC_ARR[@]}; do
        ...
        sudo perf stat -ddd -x '|' -e task-clock -e dtlb_load_misses.miss_causes_a_walk --append -o perf_results/pmem_${TLB_SIZE}_${T_LOC}_${S_LOC}.log ../wrapper-devdax.sh ${DAX_DEVICE} $((28*1024*1024*1024)) ./a.out ../output.txt
    done
done
